...
export function GET() {
  const articleIndex = engineeringArticles
    ...
    .join("");
  const body = `# ${SITE_NAME}: Public Documentation Index\n\n${SITE_DESCRIPTION}\n\nOfficial domain: ${SITE_URL}\nProject lead: ${PROJECT_LEAD} — ${PROJECT_LEAD_TITLE}\n\nEvidence boundary: ARES-01 software has been tested in local fixed-wing ArduPilot SITL. ARES Flight Computer baseline ARES-FC-REV-A-SCH-5C.0 is a bench/HIL prototype with Stage 5C schematic closure complete and is not flight approved. It has no PCB placement, routing, Gerbers, manufacturing package, fabrication order, physical sample, or flight validation. The physical aircraft, RF system, ground-station antenna, embedded-compute variant, and real-world disaster roles remain conceptual or unvalidated. No field deployment is claimed.\n\nKey pages:\n- ${SITE_URL}/engineering-log\n- ${SITE_URL}/development\n- ${SITE_URL}/documentation\n- ${SITE_URL}/roadmap\n- ${SITE_URL}/about\n- ${SITE_URL}/transparency\n- ${SITE_URL}/open-knowledge\n- ${SITE_URL}/contribute\n- ${SITE_URL}/corrections\n${articleIndex}\n`;
